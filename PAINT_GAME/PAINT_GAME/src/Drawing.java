...
public class Drawing extends JPanel implements MouseMotionListener, MouseListener, Serializable {
    ...
    private Color currentColor;  // Assuming you have a field for the current color
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Dessiner le fond d'écran si une image est définie
        if (backgroundImage != null) {
            g.drawImage(backgroundImage, 0, 0, this.getWidth(), this.getHeight(), this);
        }

        // Dessiner d'autres éléments, le cas échéant
        for (Figure f : list) {
            f.draw(g);
        }
    }
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (getNameFigure().equals("Circle") || getNameFigure().equals("Square")) {
            repaint();
            if (e.getX() < x1) {
    ...
    public void setBackgroundImage(ImageIcon imageIcon) {
        if (imageIcon != null) {
            ...
            System.err.println("ImageIcon is null");
        }
    }
}